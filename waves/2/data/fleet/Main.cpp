...
std::tuple PRIMITIVES = {
	Primitive("(%s+%s)",       +[](char x, char y)  -> char { 
		if(x == 'N' or y == 'N')  
			return 'N';
		else return i2c(c2i(x)+c2i(y));
	}),
	
//	Primitive("incr(%s)",       +[](char x)  -> char { 
//		if(x == 'N')  
//			return 'N';
//		else return i2c(c2i(x)+1);
//	}),
	Primitive("(%s-%s)",       +[](char x, char y)  -> char { 
		if(x == 'N' or y == 'N')  
			return 'N';
	...
	Primitive("(%s>%s)",       +[](char x, char y)  -> bool { 
		if(x == 'N' or y == 'N')  
			return 'N';
		else return (x>y);
	}),
	...
	Primitive("head(%s)",      +[](S s)      -> char { return (s.empty() ? 'N' : (char)s.at(0)); }),
//	Primitive("pair(%s,%s)",   +[](S a, char b) -> S { 
//			if(a.length() + 1 > MAX_LENGTH)
//				throw VMSRuntimeError;
//			else return a+S(1,b); 
//	}),
	Primitive("pair(%s,%s)",   +[](char a, S b) -> S { 
...
int main(int argc, char** argv){ 
	...
	grammar.add<S>         (BuiltinOp::op_X,            "x", 8.0); 
...
//	grammar.add<char,bool,char,char>(BuiltinOp::op_IF,           "if(%s,%s,%s)"); 
	...
	for(size_t i=0;i<alphabet.length();i++) {
		grammar.add<char>(BuiltinOp::op_ALPHABETchar, alphabet.substr(i,1), 5.0/alphabet.length(), (int)alphabet.at(i)); 
	}
